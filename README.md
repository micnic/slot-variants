...
- **`slot-variants/no-conflicting-classes`** — flags class tokens that collide within the output of an `sv()` or `cn()` call: exact duplicates and distinct tokens that target the same Tailwind-style utility namespace (e.g. `w-100` and `w-200`). It understands the structure of both calls — `base`, variants, compound variants/slots, arguments, arrays, template literals, ternaries, and logical-ANDs — and knows that mutually-exclusive positions (different values of one variant, different branches of one ternary) don't conflict. The namespace check is Tailwind-aware: overloaded prefixes are classified by property category, so `text-sm`/`text-white` or `bg-cover`/`bg-white` are fine while `text-red-500`/`text-blue-500` is reported; variant prefixes (`hover:`) and `!` markers are handled. Like `tailwind-merge`, it encodes Tailwind's default utility shapes rather than parsing your config, so a few edge cases remain (e.g. a shadeless custom color like `text-brand` is read as a size). Single-word utilities (`flex`, `block`) are ignored by default; set `exclusiveGroups: true` to enable built-in groups for `display`, `position`, `text-transform`, etc., or pass your own array of mutually-exclusive groups:
...
Only calls where `sv` or `cn` is a named import from `'slot-variants'` are analyzed. Class values hoisted into a same-file `const` are read through to their value (`const base = 'flex gap-2'; cn(base)` is analyzed as `cn('flex gap-2')`); `let`/`var`, imported bindings, and destructuring are treated as dynamic. `no-conflicting-classes` skips dynamic inputs silently to avoid false positives, while `no-dynamic-classes` flags exactly those positions — enable both for full coverage.
