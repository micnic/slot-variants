...
- **`slot-variants/no-conflicting-classes`** — flags class tokens that collide within the output of an `sv()` or `cn()` call: both exact-duplicate tokens that will appear more than once and distinct tokens that target the same Tailwind-style utility namespace (e.g. `w-100` and `w-200`). For `sv()`, detects collisions within `base`, across different variant keys, inside compound variants and compound slots, between `base` and a variant value, and within a single literal. For `cn()` (and the cn-style calling convention of `sv()` without a config), flags collisions across args, inside arrays, template literals, or within a single literal. Ternaries whose branches are both static strings (`cond ? 'w-100' : 'w-200'`) are covered too — including ones embedded in a template literal whose substitutions are each whitespace-separated from the surrounding text (`` `items-center ${col ? 'w-100' : 'w-200'}` ``, where the quasi tokens always render). Because only one branch renders, tokens from opposite branches of the same ternary are treated as mutually exclusive and don't conflict, while duplicates within a single branch — and collisions between a branch and other statically-known tokens (including a template's own quasi text) — are still flagged. Tokens with different variant prefixes (`w-100` vs `hover:w-200`) don't conflict, a leading or trailing `!` important marker (`!w-100`, `w-100!`) is ignored when computing the namespace, and tokens that only co-occur across mutually-exclusive variant values are skipped.

- **`slot-variants/no-dynamic-classes`** — flags class-bearing positions in `sv()` and `cn()` calls that aren't statically inferrable. Only string literals, template literals without expressions, flat arrays of those in config, and explicit `undefined` config class values are accepted, and config objects must use static keys (no spreads, no computed keys). In cn-style positions (`cn()` arguments and the leading arguments of `sv()`), a logical-AND ending in a static string (`cond && 'px-4'`) and a ternary whose branches are both static strings (`cond ? 'px-4' : 'px-2'`) are also accepted — the ternary may be embedded in a template literal as long as every substitution is separated from the surrounding text by whitespace (`` `items-center ${col ? 'flex-col' : 'flex-row'}` ``). A ternary with a dynamic branch, a template substitution that isn't a static-string ternary, and one whose token would straddle the substitution boundary (`` `p-${x ? '2' : '4'}` ``) are all still reported, and every conditional form remains disallowed inside an `sv()` config object. Identifiers, member access, calls, spreads, non-string literals, templates with expressions, nested config arrays, and runtime conditional records are reported. Non-class-bearing config keys (`defaultVariants`, `presets`, `requiredVariants`, `multiSlots`, `cacheSize`, `postProcess`, `introspection`) are not validated, and runtime variant matchers inside compound entries are left alone — only the `class`/`className` value (and the `slots` array of `compoundSlots`) is checked.
