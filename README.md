...
- **`slot-variants/no-conflicting-classes`** — flags class tokens that collide within the output of an `sv()` or `cn()` call: both exact-duplicate tokens that will appear more than once and distinct tokens that target the same Tailwind-style utility namespace (e.g. `w-100` and `w-200`). For `sv()`, detects collisions within `base`, across different variant keys, inside compound variants and compound slots, between `base` and a variant value, and within a single literal. For `cn()` (and the cn-style calling convention of `sv()` without a config), flags collisions across args, inside arrays, template literals, or within a single literal. Ternaries are covered too, including chained/nested ones (`a ? 'w-100' : b ? 'w-200' : 'w-300'`), branches that are themselves arrays or logical-ANDs (`cond ? ['flex', 'w-100'] : 'w-200'`), the right operand of a logical-AND (`cond && ['flex', 'w-100']`), and ternaries embedded in a template literal whose substitutions are each whitespace-separated from the surrounding text (`` `items-center ${col ? 'w-100' : 'w-200'}` ``, where the quasi tokens always render). Because exactly one branch of a ternary renders, tokens from different branches are treated as mutually exclusive and don't conflict, while duplicates and namespace clashes *within* a single branch — and collisions between a branch and other always-present tokens (a plain argument, a template's own quasi text, a logical-AND array element) — are still flagged. (One limitation: a ternary reached *through* an array or record nested inside another ternary's branch starts a fresh exclusivity scope, so exclusivity across that boundary isn't tracked.) Tokens with different variant prefixes (`w-100` vs `hover:w-200`) don't conflict, a leading or trailing `!` important marker (`!w-100`, `w-100!`) is ignored when computing the namespace, and tokens that only co-occur across mutually-exclusive variant values are skipped. Two tokens are treated as targeting the same namespace when they share a first segment **and** resolve to the same property category. For a curated set of overloaded Tailwind prefixes (`text`, `bg`, `border`, `ring`, `outline`, `divide`, `decoration`, `stroke`, `shadow`, `flex`, `font`, `grid`, `object`, `list`, `place`, `justify`, `col`, `row`, `min`, `max`, `rounded`, `backdrop`, `scroll`, `snap`, `break`, `content`, and the `x`/`y` axis utilities `gap`/`space`/`translate`/`scale`/`skew`/`rotate`/`inset`/`overflow`/`overscroll`) the value is classified into a coarse property category — color vs size vs alignment, axis vs axis, border side vs side — so `text-sm` (font size) and `text-white` (color), `bg-cover` (size) and `bg-white` (color), `flex-row` (direction) and `flex-wrap` (wrap), or `grid-cols-3` and `grid-rows-2` are *not* reported, while `text-red-500`/`text-blue-500` and `bg-white`/`bg-red-500` are. Custom-palette colors (`text-brand-500`) fall through to the color bucket without any color table. Any prefix not in that set keeps a coarser rule — same first segment and same number of dashes — which handles most single-property utilities (`w-100`/`w-200`, `bg-red-500`/`bg-blue-500`). The classifier is still approximate; a handful of edge cases remain (see below), and, like `tailwind-merge`, it encodes Tailwind's default utility shapes rather than parsing your config. Single-word utilities (`flex`, `block`, `absolute`) have no dash namespace and are ignored by default, since a project's own single-word class names would otherwise be flagged. Enable the `exclusiveGroups` option to catch these: `exclusiveGroups: true` turns on built-in Tailwind groups for `display`, `position`, `visibility`, `text-transform` (`uppercase`/`lowercase`/`capitalize`/`normal-case`), `text-decoration-line` (`underline`/`overline`/`line-through`/`no-underline`), `font-style` (`italic`/`not-italic`), `font-smoothing` (`antialiased`/`subpixel-antialiased`), `isolation` (`isolate`/`isolation-auto`), screen-reader visibility (`sr-only`/`not-sr-only`), and text-overflow (`truncate`/`text-ellipsis`/`text-clip`, where the single-word `truncate` forces ellipsis and so is exclusive with the explicit `text-*` utilities) — so `cn('flex', 'block')`, `cn('absolute', 'relative')`, `cn('uppercase', 'capitalize')`, and `cn('truncate', 'text-ellipsis')` are reported, including hyphenated siblings like `inline-block` and `line-through` that don't share a dash prefix — or pass an array of string arrays to define your own mutually-exclusive groups (e.g. `exclusiveGroups: [['on', 'off', 'auto']]`), which replaces the built-ins:
  ...
  Arbitrary color values are recognized — `text-[#f00]`, `bg-[rgb(0,0,0)]`, and the `text-[color:…]` hint classify as colors (a bracketed length like `text-[16px]` stays a size). Nested sub-properties are handled where they carry their own axis: `ring-offset-2` (width) and `ring-offset-red-500` (color) don't conflict, and `touch-pan-x`/`touch-pan-y`/`touch-pinch-zoom` compose without being flagged (while exclusive `touch-auto`/`touch-none` still conflict). Composing `-reverse` flags are recognized too — `space-x-2`/`space-x-reverse` and `divide-x-2`/`divide-x-reverse` don't conflict, while `flex-row`/`flex-row-reverse` (a direction value, not a flag) still do. Single-property color utilities (`fill`, `accent`, `caret`, `placeholder`), gradient color stops (`from`/`via`/`to`, distinguished from their `%` positions), `background-blend-mode`, `grid-auto-columns`/`grid-auto-rows`, `border-spacing-x`/`border-spacing-y` (which compose), and the `table-` prefix (splitting `table-auto`/`table-fixed` layout from `table-cell`/`table-row` display) are handled as well, as are single-property utilities with mixed value shapes (`ease-in`/`ease-in-out`, `cursor-pointer`/`cursor-not-allowed`, `origin-center`/`origin-top-right`, `align-*`, `whitespace-*`) and the Tailwind v3 `*-opacity` utilities, which compose with their color rather than conflicting. A handful of bare default utilities are mapped to their sized family too, so `rounded`/`rounded-lg`, `border`/`border-2`, `ring`/`ring-2`, `outline`/`outline-2`, `shadow`/`shadow-md`, `blur`/`blur-sm`, `grow`/`grow-0`, `shrink`/`shrink-0`, the feature toggles `transition`/`transition-none`, `transform`/`transform-gpu`, `filter`/`filter-none`, `resize`/`resize-x`, and the filter toggles `grayscale`/`grayscale-0`, `invert`/`invert-0`, `sepia`/`sepia-0`, `drop-shadow`/`drop-shadow-md` conflict, while `border`/`border-red-500` and `outline`/`outline-dashed` (a width vs a color or style) — and distinct filters like `grayscale`/`invert` — stay clear. The `size-*` shorthand is understood as overriding both axes, so it conflicts with `w-*` *and* `h-*` (`w-4`/`size-4`, `h-full`/`size-4`), while `w-*` and `h-*` still never conflict with each other — the overlap is scoped per variant prefix and is value-independent (`w-full`/`size-4` conflict). One structural limitation remains, and it's the *safe* direction (a missed collision, never a crash): a custom color defined without a shade (a single-segment `text-brand`) is read as a size, because the classifier can't tell a bespoke color name from a bespoke size name without reading your Tailwind config.

- **`slot-variants/no-dynamic-classes`** — flags class-bearing positions in `sv()` and `cn()` calls that aren't statically inferrable. Only string literals, template literals without expressions, flat arrays of those in config, and explicit `undefined` config class values are accepted, and config objects must use static keys (no spreads, no computed keys). In cn-style positions (`cn()` arguments and the leading arguments of `sv()`), the conditional forms of the `cn()` calling convention are also accepted and may nest arbitrarily: a logical-AND whose right operand is a static class value (`cond && 'px-4'`, `cond && ['flex', 'px-4']`, `cond && { 'px-4': on }`), a ternary whose branches are each a static class value (`cond ? 'px-4' : 'px-2'`, `cond ? ['flex', 'px-4'] : 'px-2'`, chained as `a ? 'x' : b ? 'y' : 'z'`), and a ternary embedded in a template literal as long as every substitution is a static-string ternary separated from the surrounding text by whitespace (`` `items-center ${col ? 'flex-col' : 'flex-row'}` ``). A branch or operand that isn't itself static is reported at that position; a template substitution that isn't a static-string ternary, one that would let a token straddle the substitution boundary (`` `p-${x ? '2' : '4'}` ``), and one containing a non-string form like a logical-AND (which stringifies to `"false"` when skipped) leave the whole template reported. Every conditional form remains disallowed inside an `sv()` config object. Identifiers, member access, calls, spreads, non-string literals, templates with expressions, nested config arrays, and runtime conditional records are reported. Non-class-bearing config keys (`defaultVariants`, `presets`, `requiredVariants`, `multiSlots`, `cacheSize`, `postProcess`, `introspection`) are not validated, and runtime variant matchers inside compound entries are left alone — only the `class`/`className` value (and the `slots` array of `compoundSlots`) is checked.
...
Only calls where `sv` or `cn` is a named import from `'slot-variants'` are analyzed. Class values hoisted into a same-file `const` are read through to their value (`const base = 'flex gap-2'; cn(base)` is analyzed as `cn('flex gap-2')`), including chains of `const` bindings and references nested inside arrays, records, or ternary branches, so `no-dynamic-classes`, `no-conflicting-classes`, and `no-redundant-spaces` all treat the constant as its value. Only a single `const name = <value>` binding (which can't be reassigned) is followed — `let`/`var`, redeclarations, imported bindings (a different module), destructuring, and reference cycles are left as dynamic. `no-conflicting-classes` skips dynamic inputs silently to avoid false positives; `no-dynamic-classes` is the opposite — it flags exactly those positions so the static analyzer can fully reason about every call. `no-shared-tokens` sits between them: it needs a fully statically analyzable, exhaustive variant before it can prove a token is constant across every value. `no-empty-classes` and `no-redundant-spaces` are independent and complement the structural rules: they cover empty and badly-shaped literals reachable from a call's arguments, regardless of whether the surrounding call is fully static.
